...
from tensorflow.keras.layers import (
    ...
    Activation, ConvLSTM2D, ReLU, Dense, Multiply, Permute, Flatten, Dropout
)
...
class sstModel(tf.keras.Model):
    def __init__(self):
        ...
        self.input_layer = Input(shape=(6, 40, 92, 72, 1))
        self.inception = InceptionBlk(19)
        self.att = GAMAttention(184)

        self.ConvLSTM2D0 = ConvGRU2D(filters=32, kernel_size=3, strides=1, padding='same', return_sequences=True)
        self.ConvLSTM2D = ConvGRU2D(filters=24, kernel_size=3, strides=1, padding='same', return_sequences=False)
...
import xarray as xr
def loadData(rate=0.1, window_size=50):
    path_sst = r'D:\zyh\dataset\OISST\oisst_daily_2014_2023.nc'
    # 使用 xarray 打开文件并读取数据
    ...
    # 提取目标区域内的数据
    sst_da = ds_sst['sst'].sel(lon=slice(target_lon_range[0], target_lon_range[1]),
                               lat=slice(target_lat_range[0], target_lat_range[1]))
    sst_da = sst_da.squeeze()  # 这将移除所有长度为1的维度
    print(f"sst shape after slicing: {sst_da.shape}")

    # 在转换为 numpy 数组之前获取坐标值
    lon = sst_da['lon'].values
    lat = sst_da['lat'].values

    # 转换为 numpy 数组
    sst = sst_da.values.astype(float)
    # 处理缺失值
    sst[np.isnan(sst)] = 0  # 或者使用其他方法处理缺失值
...
def customized_r2(y_true, y_pred):
    sst_true = y_true[..., 0].flatten()
    sst_pred = y_pred[..., 0].flatten()
    mask_sea = (sst_true != 0)

    if not np.any(mask_sea):
        return 0.0
    return r2_score(sst_true[mask_sea], sst_pred[mask_sea])
def customized_pacc(y_true, y_pred):
    """
    计算 PACC 指标
    PACC = 1 - mean(|(y_true - y_pred) / y_true|)
    只在海洋区域（y_true != 0）计算，避免除以零
    """
    sst_true = y_true[..., 0]  # 形状: (lat, lon)
    sst_pred = y_pred[..., 0]
    ...
    # 提取有效区域
    true_flat = sst_true[mask]
    pred_flat = sst_pred[mask]

    # 避免除以零：过滤掉 true_flat 中为 0 的点（虽然 mask 已过滤）
    non_zero = (true_flat != 0)
    if not np.any(non_zero):
        return 0.0

    relative_errors = np.abs((true_flat[non_zero] - pred_flat[non_zero]) / true_flat[non_zero])
    mean_relative_error = np.mean(relative_errors)

    pacc = 1 - mean_relative_error
    return pacc
...
if __name__ == '__main__':
    ...
    x_train, y_train, x_val, y_val, x_test, y_test, max_sst, min_sst, sst, lon, lat = loadData(0.1)
    ...
    with strategy.scope():
        ...
        model = sstModel()
        # 使用一些虚拟输入调用模型以构建模型结构
        dummy_input = np.zeros((GLOBAL_BATCH_SIZE, 50, 92, 72, 1))  # 根据您的模型调整输入形状
        _ = model(dummy_input)
        model.load_weights(f"D:\zyh\TTTT\weight\TTTT-NOAA-30\weights.h5")
    '''
if __name__ == '__main__':
    dt_start = datetime.now()
    # 设置可见的GPU设备
    ...
    with strategy.scope():
        ...
        lr_metric = get_lr_metric(optimizer)  # 用于获取学习率
        model = sstModel()  # 创建模型实例
        model.build(input_shape=(GLOBAL_BATCH_SIZE, 50, 92, 72, 1))  # 使用全局批大小构建模型
        ...
        filepath = f"D:\zyh\TTTT\weight\TTTT-NOAA-30\weights.h5"
        ...
        history = model.fit(generator_data(x_train, y_train, GLOBAL_BATCH_SIZE),
                            steps_per_epoch=max(1, x_train.shape[0] // GLOBAL_BATCH_SIZE), epochs=100,
...
'''
    ...
    rmse_list, mae_list, r2_list = [[] for _ in range(num_days_to_predict)], [[] for _ in range(num_days_to_predict)], [
        [] for _ in range(num_days_to_predict)]
    pacc_list = [[] for _ in range(num_days_to_predict)]
    for i in range(x_test.shape[0] - num_days_to_predict):
        ...
        for day in range(num_days_to_predict):
            ...
            y_test_denorm = y_test[i + day] * (max_sst - min_sst) + min_sst

            # 计算各项指标
            rmse = customized_rmse(y_test_denorm, predicted_sst_denorm)
            mae = customized_mae(y_test_denorm, predicted_sst_denorm)
            r2 = customized_r2(y_test_denorm, predicted_sst_denorm)
            pacc = customized_pacc(y_test_denorm, predicted_sst_denorm)  # 新增：PACC

            # 存储结果
            rmse_list[day].append(rmse)
            mae_list[day].append(mae)
            r2_list[day].append(r2)
            pacc_list[day].append(pacc)  # 存储 PACC

            # 打印当前结果
            print(f"对于样本索引{i}，第{day + 1}天的预测：")
            print(f"RMSE: {rmse:.6f}")
            print(f"MAE: {mae:.6f}")
            print(f"R²: {r2:.6f}")
            print(f"PACC: {pacc:.6f}")
    for day in range(num_days_to_predict):
        avg_rmse = np.mean(rmse_list[day])
        avg_mae = np.mean(mae_list[day])
        avg_r2 = np.mean(r2_list[day])
        avg_pacc = np.mean(pacc_list[day])  # 计算平均 PACC

        print(f"Day {day + 1}:")
        print(f"  Average RMSE: {avg_rmse:.6f}")
        print(f"  Average MAE: {avg_mae:.6f}")
        print(f"  Average R²: {avg_r2:.6f}")
        print(f"  Average PACC: {avg_pacc:.6f}")
...
'''
    ...
    for i in range(x_test.shape[0] - num_days_to_predict):
        ...
        predicted_sst_denorm = np.squeeze(predictions[4] * (max_sst - min_sst) + min_sst)
        true_sst_denorm = np.squeeze(y_test[i + num_days_to_predict - 1] * (max_sst - min_sst) + min_sst)
        lower_threshold = 12.399999618530273
        upper_threshold = 32.68000030517578
    ...
    im = ax.imshow(average_rmse_per_grid, transform=ccrs.PlateCarree(), cmap='jet', interpolation='bilinear',
                   extent=[lon.min(), lon.max(), lat.min(), lat.max()], vmin=0.205, vmax=2.060,
    ...
    plt.savefig(os.path.join(directory, 'TTTT-Average_RMSE_5th_Day.png'),dpi=600, bbox_inches='tight', pad_inches=0.1)  # 保存图像
    plt.close()
    ...
    im = ax.imshow(average_mae_per_grid, transform=ccrs.PlateCarree(), cmap='jet', interpolation='bilinear',
                   extent=[lon.min(), lon.max(), lat.min(), lat.max()], vmin=0.165, vmax=1.277,
                   origin='lower')  # 添加此行以修正图像方向
    ...
    plt.tight_layout()  # 确保子图之间不会重叠
    plt.savefig(os.path.join(directory, 'TTTT-Average_MAE_5th_Day.png'),dpi=600, bbox_inches='tight', pad_inches=0.1)  # 保存图像
    plt.close()  # 关闭图像
...
rmse_list, mae_list, r2_list, corr_list = [[] for _ in range(num_days_to_predict)], \
                                          ...
                                          [[] for _ in range(num_days_to_predict)], \
                                          [[] for _ in range(num_days_to_predict)]
prediction_list = [[] for _ in range(num_days_to_predict)]  # 用于存储每天的预测值
# 初始化列表用于存储每天的最大和最小值
...
for i in range(x_test.shape[0] - num_days_to_predict):
    ...
    for day in range(num_days_to_predict):
        ...
        prediction_list[day].append(predicted_sst_denorm)  # 保存原始形状的预测值
        # 记录真实值和预测值的最大和最小值
...
for day in range(num_days_to_predict):
    print(f"Day {day + 1}:")
    print(f"True Values Max: {np.max(true_max_values[day]):.2f}, Min: {np.min(true_min_values[day]):.2f}")
    print(f"Predicted Values Max: {np.max(pred_max_values[day]):.2f}, Min: {np.min(pred_min_values[day]):.2f}")
...
for day in range(num_days_to_predict):
    ...
    true_sst_flatten = np.concatenate(
        [y_test[i + day] * (max_sst - min_sst) + min_sst for i in range(x_test.shape[0] - num_days_to_predict + 1)])
    pred_sst_flatten = np.concatenate(prediction_list[day])
    ...
    hb = ax.hexbin(true_sst_flatten[mask_sea], pred_sst_flatten[mask_sea], gridsize=400, cmap=cmap,
                   bins='log',vmin=1, vmax=1352)  # 确保颜色条的范围也对应于温度区间
    # 获取对数变换后的颜色条范围
    ...
    # 设置子图标题
    ax.set_title(f'Day {day + 1}', fontproperties=font, fontsize=22)
    ...
    # 设置横纵坐标的刻度间隔为5
    ax.set_xticks(range(15, 36, 5))  # 设置x轴刻度间隔为5
    ax.set_yticks(range(15, 36, 5))  # 设置y轴刻度间隔为5
    ...
    output_folder = 'TTTT-scatter_plots-30'
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    # 保存图表到文件，为每一天指定不同的文件名
    filename = os.path.join(output_folder, f'TTTT_plot_day_{day + 1}.png')
