...
class sstModel(tf.keras.Model):
    def __init__(self, return_attention=False):
        ...
        self.ConvLSTM2D0 = ConvGRU2D(filters=32, kernel_size=3, strides=1,
                                     padding='same', return_sequences=True)
        self.ConvLSTM2D  = ConvGRU2D(filters=24, kernel_size=3, strides=1,
...
if __name__ == '__main__':
    ...
    save_root = "./attention_maps"  # 可自行修改
    ...
    for i in range(x_test.shape[0] - num_days_to_predict):
        print(f"当前索引 i: {i}")
        x_test_sample = x_test[i:i + 1]  # shape: (1, 50, 92, 72, 1) -> numpy.ndarray
        ...
        # 计算 50 天平均注意力图
        # -------------------------------
        att_50_mean = np.mean(spa_att[0].numpy(), axis=0)  # (92, 72, 1)
        att_50_mean = np.squeeze(att_50_mean)  # (92, 72)

        # -------------------------------
        # 使用 SST 均值创建陆地掩膜
        # -------------------------------
        sst_50_mean = np.mean(x_test_sample[0, :, :, :, 0], axis=0)  # (92, 72)
        land_mask = (sst_50_mean == 0)
        att_50_mean = np.ma.masked_where(land_mask, att_50_mean)

        proj = ccrs.PlateCarree()
        fig = plt.figure(figsize=(6, 5))  # 调整大小以适应单图
        ax = plt.axes(projection=proj)

        im = ax.imshow(att_50_mean, extent=[lon.min(), lon.max(), lat.min(), lat.max()],
        ...
        # ax.set_title('Mean Spatial Attention (50 days)', fontsize=14, pad=20)
